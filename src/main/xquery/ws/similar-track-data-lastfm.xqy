...
declare function local:create-uri($artist as xs:string, $track as xs:string){
  ("/lfm/"||xdmp:url-encode($artist, fn:true())||"/"||xdmp:url-encode($track, fn:true())||".xml")
};

declare function local:has-meta($artist as xs:string, $track as xs:string) as xs:boolean {
  fn:local-name(fn:doc(local:create-uri($artist, $track))/node()) eq $config:LAST-FM-SIMILAR-TRACK-ROOT-XML-ELEMENT
};
